...
import com.qishanor.common.excel.vo.ErrorMessage;
...
//
//	private final SysDeptMapper userDeptMapper;
	 ...
	 */
	@Override
	public List<DeptExcelVo> exportDeptExcel() {

		List<SysDept> list = this.list();
		// 优化：先构建 deptId -> name 的 Map，避免在循环中重复查询
		Map<Long, String> deptNameMap = list.stream()
		...
		List<DeptExcelVo> deptExcelVos = list.stream().map(item -> {
			...
			// 从 Map 中查找父部门名称
			String parentName = deptNameMap.get(item.getParentId());
			deptExcelVo.setParentName(parentName != null ? parentName : "根部门");
	...
	@Override
	public R importDeptExcel(List<DeptExcelVo> excelVoList) {
		List<ErrorMessage> errorMessageList = new ArrayList<ErrorMessage>();


		// 一次性查询所有部门，构建 Map 避免重复查询
		List<SysDept> deptList = this.list();
		// 构建部门名称 -> 部门的 Map，用于快速查找
		Map<String, SysDept> deptNameMap = deptList.stream()
				.collect(Collectors.toMap(
						SysDept::getName,
						dept -> dept,
		...
		// 构建部门名称集合，用于快速判断是否存在
		Set<String> deptNameList = deptList.stream()
				.map(SysDept::getName)
				.collect(Collectors.toSet());

		// 收集需要插入的部门
		List<SysDept> deptToInsert = new ArrayList<>();
		...
		for (DeptExcelVo item : excelVoList) {
			...
			// 验证部门名称是否已存在
			if (deptNameList.contains(item.getName())) {
				errorMsg.add(String.format("%s 部门名称已经存在",item.getName()));
			}

			// 处理父部门逻辑
			Long parentId=null;
			if ("根部门".equals(item.getParentName())) {
				// 根部门的 parentId 为 0
				parentId = 0L;
			}

			// 从 Map 中查找父部门
			SysDept parentDept = deptNameMap.get(item.getParentName());
			if (parentDept == null) {
				errorMsg.add(String.format("%s 上级部门不存在",item.getName()));
			} else {
				parentId = parentDept.getDeptId();
			}
		...
		// 批量插入部门
		if (CollUtil.isNotEmpty(deptToInsert)) {
			this.saveBatch(deptToInsert);
		}
...
//
//        List<SysDept> userDeptList = userDeptMapper
//                .selectList(Wrappers.<SysDept>lambdaQuery().eq(SysDept::getDeptId, deptId));
...
////		}
//
//		List<Long> userIdList = userDeptList.stream().map(SysDept::getUserId).toList();
