<template>
	<el-dialog :title="dataForm.dictId ? $t('common.editBtn') : $t('common.addBtn')" v-model="visible" width="600">
...
const onSubmit = async () => {
	...
	try {
		...
		const result = dataForm.dictId ? await putObj(dataForm) : await addObj(dataForm);
		useMessage().success(t(dataForm.dictId ? 'common.editSuccessText' : 'common.addSuccessText'));
		visible.value = false;
