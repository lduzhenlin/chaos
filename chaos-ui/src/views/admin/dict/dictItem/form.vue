<template>
  <el-dialog v-model="visible" :title="dataForm.itemId ? $t('common.editBtn') : $t('common.addBtn')" width="600">
...
// 打开弹窗
const openDialog = (itemId: any, dictForm: any) => {
  visible.value = true;
  dataForm.itemId = '';
...
const onSubmit = async () => {
  ...
  try {
    loading.value = true;
    dataForm.itemId ? await putItemObj(dataForm) : await addItemObj(dataForm);
    useMessage().success(t(dataForm.itemId ? 'common.editSuccessText' : 'common.addSuccessText'));
    visible.value = false;
    emit('refresh');
  } catch (err: any) {
