		...
		>
			...
			<el-table-column :label="$t('common.action')" width="150">
				<template #default="scope">
					<el-button icon="edit-pen" text type="primary" @click="dictformRef.openDialog(scope.row.itemId,queryForm)"> {{ $t('common.editBtn') }} </el-button>
					<el-button icon="delete" text type="primary" @click="handleDelete(scope.row.itemId)">
						{{ $t('common.delBtn') }}
					</el-button>
				</template>
			</el-table-column>
...
const open = (row: any) => {
	state.queryForm.dictId = row.dictId;
