				...
				<el-row :gutter="20">
					...
					<el-col :span="12" class="mb20">
						<el-form-item :label="$t('sysuser.phone')" prop="tel">
							<el-input clearable :placeholder="$t('sysuser.inputPhoneTip')" v-model="dataForm.tel"></el-input>
						</el-form-item>
					</el-col>
					<el-col :span="12" class="mb20">
						<el-form-item :label="$t('sysuser.role')" prop="role">
							<el-select clearable multiple :placeholder="$t('sysuser.selectRole')" v-model="dataForm.role">
								<el-option :key="item.roleId" :label="item.name" :value="item.roleId" v-for="item in roleData" />
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12" class="mb20">
...
const dataForm = reactive({
	...
	qqOpenid: '',
	lockFlag: '0',
	tel: '' as String | undefined,
	deptId: '',
	roleList: [],//后台返回值当前用户数据
	postList: [],
	nickname: '',
	name: '',
	email: '',
	post: [] as string[], //提交给后台数据
	role: [] as string[],
});
...
const dataRules = ref({
	...
	username: [
		...
		{ min: 1, max: 20, message: t('sysuser.usernameLength'), trigger: 'blur' },
		// {
...
const getUserData = async (id: string) => {
	try {
		...
		originalPhone.value = data.tel; // 保存原始手机号
		if (data.roleList) {
